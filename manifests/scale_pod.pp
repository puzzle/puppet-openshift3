define openshift3::scale_pod ($namespace = 'default', $pod = $title, $replicas) {
  ...
  exec { "Scale pod ${pod} in namespace ${namespace} to ${replicas} replicas":
    ...
    cwd         => "/root",
    command     => "READY_NODES=`oc get nodes -l logging-infra-fluentd=true -o name 2>/dev/null| wc -l` && \
                    oc scale -n ${namespace} dc ${pod} --replicas=${real_replicas}",
    unless      => "! oc get -n ${namespace} dc ${pod} >/dev/null 2>&1 || ( READY_NODES=`oc get nodes -l logging-infra-fluentd=true -o name 2>/dev/null | wc -l` && \
