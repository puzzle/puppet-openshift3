define openshift3::scale_pod ($namespace = 'default', $pod = $title, $replicas) {
  ...
  exec { "Scale pod ${pod} in namespace ${namespace} to ${replicas} replicas":
    ...
    unless      => "! oc get -n ${namespace} dc ${pod} >/dev/null 2>&1 || ( READY_NODES=`oc get nodes -o json 2>/dev/null |jq -r '.items[] | select(.spec.unschedulable!=true and .status.conditions[0].status==\"True\").spec.externalID' 2>/dev/null | wc -l` && \
